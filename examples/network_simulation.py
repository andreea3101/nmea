...
def main():
    ...
    tcp_enabled = any(
        oc.type == "tcp" and oc.enabled for oc in config.output_configs)
    if tcp_enabled:
        tcp_thread = threading.Thread(
            target=tcp_client_test, args=("localhost", 10110, 60), daemon=True
        )
        client_threads.append(tcp_thread)

    # Check if UDP output is configured
    udp_enabled = any(
        oc.type == "udp" and oc.enabled for oc in config.output_configs)
    if udp_enabled:
        udp_thread = threading.Thread(
            target=udp_client_test, args=("localhost", 10111, 60), daemon=True
        )
