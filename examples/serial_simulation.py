...
# Setup a pair (e.g., COM10 and COM11) and configure SERIAL_PORT_NAME
# accordingly.

# Adjust if your virtual port is different
SERIAL_PORT_NAME = "/tmp/vserial_sim_tx"
...
def main():
    ...
    print("=" * 50)
    print(
        f"INFO: This script will attempt to output NMEA data to serial port: {SERIAL_PORT_NAME}"
    )
    print(
        "INFO: Ensure a virtual serial port is set up (see comments in script) or a physical port is available."
    )
    print(
        "INFO: To read the output, you can use 'cat /tmp/vserial_sim_rx' (if using socat example setup)."
    )
    print("-" * 50)
    ...
    if hasattr(config, "output_configs"):
        output_handlers = OutputFactory.create_output_handlers(
    ...
    # The "client" is an external program reading the other end of the serial
    # port.
    ...
    try:
        ...
        while engine.running:
            ...
            for i, handler_status in enumerate(status["output_handlers"]):
                ...
                if (
                    "port" in handler_status and "baudrate" in handler_status
                ):  # Serial specific
                    handler_info = f"Serial ({
                        handler_status['port']}@{
                        handler_status['baudrate']})"
                    ...
                    handler_info = f"UDP (targets: {
                        len(
                            handler_status['targets'])})"
                ...
                if "is_open" in handler_status:  # Serial specific
                    print(
                        f"    Port Open: {
                            handler_status['is_open']}, Reconnecting: {
                            handler_status.get(
                                'reconnecting',
                                False)}"
                    )
        ...
        final_status = (
            engine.get_status()
        )  # Get final status again after engine stop might update it
        ...
        if stats["runtime_seconds"] > 0:
            print(
                f"  Average rate: {
                    stats['sentences_per_second']:.1f} sentences/second"
            )
