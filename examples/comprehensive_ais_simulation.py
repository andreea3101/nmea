...
from simulator.config.multi_vessel import (
    MultiVesselConfigManager,
    create_san_francisco_bay_scenario,
)
...
def main():
    ...
    print(
        f"  Trace Logging: {
            'Enabled' if config.enable_trace_logging else 'Disabled'}"
    )
    print()
    ...
    trace_logger = create_trace_logger(
        log_file="logs/ais_trace.jsonl", enable_console=False
    )
    ...
    scenario = config_manager.create_fleet_scenario(
        "sf_bay_demo", sf_scenario_config)
    ...
    for vessel_config in scenario.vessels:
        ...
        print(
            f"  {
                vessel_config['name']} (MMSI: {mmsi}) at {
                pos['latitude']:.4f}, {
                pos['longitude']:.4f}"
        )
    ...
    output_configs = [{"type": "file",
                       "config": {"filename": "logs/nmea_ais_output.log",
                                  "max_size_mb": 10,
                                  "backup_count": 3,
                                  },
                       },
    ...
    # Alias OutputConfig from parser to avoid name clash if needed, or ensure
    # distinct usage
        ...
        try:
            ...
            if output_type == "file":
                typed_config = FileOutputConfig(
                    ...
                    file_path=specific_config_dict["filename"],
                ...
                typed_config = UDPOutputConfig(
                    ...
                    broadcast=specific_config_dict.get(
                        "broadcast",
                        True),
            ...
            if typed_config:
                ...
                handler = OutputFactory.create_output_handler(
                    parser_config_obj)
                ...
                print(
                    f"  ⚠️ {
                        output_setting['type'].upper()}: Could not create typed config, skipping.")
    ...
    try:
        while simulation_running and engine.is_running():
            ...
            if (current_time - last_stats_time).total_seconds() >= stats_interval:
                ...
                print(
                    f"Sentences sent: {
                        stats['sentences_sent']} ({
                        stats['sentences_per_second']:.1f}/s)"
                )
                ...
                if ais_stats.get("message_types"):
                    ...
                    for msg_type, type_stats in ais_stats["message_types"].items(
                        ...
                        print(
                            f"  Type {msg_type}: {
                                type_stats['total_sent']} sent ({
                                type_stats['description']})"
                        )
    ...
    print(
        f"Average rate: {
            final_stats['sentences_per_second']:.1f} sentences/second"
    )
    ...
    output_files = ["logs/nmea_ais_output.log", "logs/ais_trace.jsonl"]
    ...
    print(
        "  python3 -c \"from simulator.core.trace_logger import analyze_trace_file; analyzer = analyze_trace_file('logs/ais_trace.jsonl'); print(analyzer.get_message_type_stats())\""
    )
    print()
