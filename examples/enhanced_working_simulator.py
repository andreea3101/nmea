...
class EnhancedNMEASimulator:
    ...
    def generate_scenario(self, duration_minutes=10):
        ...
        print(
            f"Generating {duration_minutes}-minute scenario with {len(self.vessels)} vessels..."
        )
        ...
        with open(nmea_file, "w") as nmea_f, open(human_file, "w") as human_f:
            ...
            while current_time < end_time:
                    ...
                    for vessel in self.vessels:
                        ...
                        for sentence in gps_sentences:
                            ...
                            self._write_human_readable(
                                human_f, sentence, "GPS", current_time, vessel
                            )
                ...
                if step % 10 == 0:
                    for vessel in self.vessels:
                        try:
                            ais_sentences = self._generate_ais_type1(
                                vessel, current_time
                            )
                            for sentence in ais_sentences:
                                ...
                                # Add reference data
                                self._add_reference_data(
                                    sentence, "AIS", current_time, vessel.mmsi, 1)
                                self._write_human_readable(
                                    human_f, sentence, "AIS", current_time, vessel, 1)
                        except Exception as e:
                            print(
                                f"Error generating AIS for vessel {
                                    vessel.mmsi}: {e}"
                            )
                ...
                if step % 60 == 0:
                    for vessel in self.vessels:
                        if vessel.static_data.vessel_class == VesselClass.CLASS_A:
                            try:
                                ais_sentences = self._generate_ais_type5(
                                    vessel, current_time
                                )
                                for sentence in ais_sentences:
                                    # Write to file
                                    ...
                                    # Add reference data
                                    self._add_reference_data(
                                        sentence, "AIS", current_time, vessel.mmsi, 5)
                                    self._write_human_readable(
                                        human_f,
                                        sentence,
                                        "AIS",
                                ...
                                print(
                                    f"Error generating AIS Type 5 for vessel {
                                        vessel.mmsi}: {e}"
                                )
                ...
                if step % 60 == 0:
                    ...
                    tcp_clients = (len(self.network.tcp_clients)
                                   if self.network.tcp_clients else 0)
                    print(
                        f"Progress: {
                            progress:.1f}% - Generated {
                            self.message_count} messages - TCP clients: {tcp_clients}")
    ...
    def _update_vessel_positions(self, current_time, start_time):
        ...
        for vessel in self.vessels:
            ...
            lat_change = (distance_m * 0.000009) * (
                1 if vessel.navigation_data.cog < 180 else -1
            )
            lon_change = (distance_m * 0.000009) * (
                1 if 90 < vessel.navigation_data.cog < 270 else -1
            )
    ...
    def _add_reference_data(
        self, sentence, msg_type, timestamp, vessel_mmsi, ais_msg_type=None
    ):
    ...
    def _write_human_readable(
        self, file, sentence, msg_type, timestamp, vessel, ais_msg_type=None
    ):
        ...
        if msg_type == "GPS":
            if sentence.startswith("$GPGGA"):
                file.write(
                    f"GPS Fix - {vessel.static_data.vessel_name} (MMSI: {vessel.mmsi})\n"
                )
                file.write(
                    f"  Position: {
                        nav.position.latitude:.6f}, {
                        nav.position.longitude:.6f}\n"
                )
            elif sentence.startswith("$GPRMC"):
                file.write(f"GPS RMC - {vessel.static_data.vessel_name}\n")
                file.write(
                    f"  Speed: {
                        nav.sog:.1f} knots, Course: {
                        nav.cog:.1f}°\n"
                )

        elif msg_type == "AIS":
            file.write(
                f"AIS Type {ais_msg_type} - {vessel.static_data.vessel_name} (MMSI: {vessel.mmsi})\n"
            )
            if ais_msg_type == 1:
                file.write(
                    f"  Position Report: {
                ...
                file.write(
                    f"  Static Data: {
                        vessel.static_data.vessel_name}\n"
                )
                file.write(
                    f"  Call Sign: {
                        vessel.static_data.callsign}, Destination: {
