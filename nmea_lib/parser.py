...
from .base import TalkerId, SentenceId, ParsedSentence
...
class SentenceParser:
    ...
    def _parse(self) -> None:
        ...
        if not talker_str or not sentence_str:
            raise ValueError(
                f"Cannot parse sentence components: {
                    self.raw_sentence}"
            )
    ...
    def set_field(self, index: int, value: str) -> None:
        ...
        self.parsed_data.fields[index] = str(
    ...
    def set_float_field(
        self, index: int, value: Optional[float], precision: int = 1
    ) -> None:
    ...
    def build_sentence(self) -> str:
        ...
        header = (
            f"${self.parsed_data.talker_id.value}{self.parsed_data.sentence_id.value}"
        )
        ...
        sentence_body = f"{
            self.parsed_data.talker_id.value}{
            self.parsed_data.sentence_id.value},{fields_str}"
...
class SentenceBuilder:
    ...
    def add_field(self, value: Union[str, int,
                  float, None]) -> "SentenceBuilder":
    ...
    def add_float_field(
        self, value: Optional[float], precision: int = 1
    ) -> "SentenceBuilder":
    ...
    def build(self) -> str:
        ...
        # Build sentence body
        sentence_body = (
            f"{self.talker_id.value}{self.sentence_id.value},{','.join(self.fields)}"
        )
