...
class VesselDimensions:
    ...
    # Distance from reference point to starboard (meters)
    to_starboard: int = 0
...
class VesselState:
    ...
    def should_send_message(
        self, message_type: int, current_time: datetime, interval: float
    ) -> bool:
        ...
        elapsed = (
            current_time -
            self.last_message_times[message_type]).total_seconds()
    ...
    def mark_message_sent(
        ...
        self.message_sequence[message_type] = (
            self.message_sequence[message_type] + 1
        ) % 4
    ...
    def validate(self) -> bool:
        ...
        return (
            self.static_data.validate()
            and self.navigation_data.validate()
            and self.voyage_data.validate()
        )


# Factory functions for creating vessel states
def create_vessel_state(
    mmsi: int,
    vessel_name: str,
    position: Position,
    vessel_class: VesselClass = VesselClass.CLASS_A,
    **kwargs
    ...
    static_data = VesselStaticData(
        ...
        **{
            ...
            if k in VesselStaticData.__dataclass_fields__
        }
    )
    ...
    navigation_data = VesselNavigationData(
        ...
        **{
            k: v
            for k, v in kwargs.items()
            if k in VesselNavigationData.__dataclass_fields__
        }
    )
    ...
    voyage_data = VesselVoyageData(
        **{
            k: v
            for k, v in kwargs.items()
            if k in VesselVoyageData.__dataclass_fields__
        }
    )
...
def create_base_station(
        mmsi: int,
        position: Position,
        **kwargs) -> BaseStationData:
    """Create a base station with default values."""
    return BaseStationData(
        mmsi=mmsi, position=position, timestamp=datetime.now(), **kwargs
    )


def create_aid_to_navigation(
    mmsi: int, name: str, position: Position, aid_type: int = 0, **kwargs
) -> AidToNavigationData:
    """Create an aid to navigation with default values."""
    return AidToNavigationData(
        mmsi=mmsi, name=name, position=position, aid_type=aid_type, **kwargs
    )
