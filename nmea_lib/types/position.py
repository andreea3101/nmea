...
class Position:
    ...
    @classmethod
    def from_nmea(
        cls, lat_str: str, lat_hem: str, lon_str: str, lon_hem: str
    ) -> "Position":
    ...
    def to_nmea(self) -> Tuple[str, str, str, str]:
        ...
        lat_hem = (Hemisphere.NORTH.value if self.latitude >=
                   0 else Hemisphere.SOUTH.value)
        ...
        lon_hem = (Hemisphere.EAST.value if self.longitude >=
                   0 else Hemisphere.WEST.value)
    ...
    def move_by_bearing_distance(
        self, bearing_deg: float, distance_m: float
    ) -> "Position":
    ...
    def __str__(self) -> str:
        ...
        return (
            f"{abs(self.latitude):.6f}°{lat_hem}, {abs(self.longitude):.6f}°{lon_hem}"
        )
