...
from nmea_lib.ais.constants import AIS_NOT_AVAILABLE, AIS_MAX_VALUES
...
class AISBinaryEncoder:
    ...
    def _encode_latitude(lat: float) -> str:
        ...
        if lat == AIS_NOT_AVAILABLE["latitude"]:
            return AISBinaryEncoder._encode_bits(
                0x3412140, 27)  # Not available
    ...
    def encode_type_1(vessel: VesselState) -> Tuple[str, Dict[str, Any]]:
        ...
        binary += AISBinaryEncoder._encode_bits(
            nav.nav_status.value, 4
        )  # Navigation status
        binary += AISBinaryEncoder._encode_rot(nav.rot)  # Rate of turn
        binary += AISBinaryEncoder._encode_sog(nav.sog)  # Speed over ground
        binary += AISBinaryEncoder._encode_bits(
            nav.position_accuracy, 1
        )  # Position accuracy
        binary += AISBinaryEncoder._encode_longitude(
            nav.position.longitude
        )  # Longitude
        # Latitude
        binary += AISBinaryEncoder._encode_latitude(nav.position.latitude)
        ...
        # Radio status
        binary += AISBinaryEncoder._encode_bits(nav.radio_status, 19)
    ...
    @staticmethod
    def encode_type_4(
            base_station: BaseStationData) -> Tuple[str, Dict[str, Any]]:
        ...
        binary += AISBinaryEncoder._encode_bits(1, 1)
        binary += AISBinaryEncoder._encode_longitude(
            base_station.position.longitude
        )  # Longitude
        binary += AISBinaryEncoder._encode_latitude(
            base_station.position.latitude
        )  # Latitude
        binary += AISBinaryEncoder._encode_bits(
            base_station.epfd_type.value, 4
        )  # EPFD type
        binary += AISBinaryEncoder._encode_bits(0, 10)  # Spare
        binary += AISBinaryEncoder._encode_bits(base_station.raim, 1)  # RAIM
        binary += AISBinaryEncoder._encode_bits(
            base_station.radio_status, 19
        )  # Radio status
    ...
    def encode_type_5(vessel: VesselState) -> Tuple[str, Dict[str, Any]]:
        ...
        binary += AISBinaryEncoder._encode_bits(
            static.imo_number or 0, 30
        )  # IMO number
        binary += AISBinaryEncoder._encode_string(
            static.callsign, 7
        )  # Call sign (42 bits)
        binary += AISBinaryEncoder._encode_string(
            static.vessel_name, 20
        )  # Vessel name (120 bits)
        # Ship type
        binary += AISBinaryEncoder._encode_bits(static.ship_type.value, 8)

        # Dimensions
        dims = static.dimensions.to_ais_format()
        binary += AISBinaryEncoder._encode_bits(dims[0], 9)  # Dimension to bow
        # Dimension to stern
        binary += AISBinaryEncoder._encode_bits(dims[1], 9)
        # Dimension to port
        binary += AISBinaryEncoder._encode_bits(dims[2], 6)
        # Dimension to starboard
        binary += AISBinaryEncoder._encode_bits(dims[3], 6)

        # EPFD type
        binary += AISBinaryEncoder._encode_bits(static.epfd_type.value, 4)
        ...
        draught_int = int(
            round(
                voyage.draught *
                10)) if voyage.draught < 25.5 else 0
        # Maximum draught
        binary += AISBinaryEncoder._encode_bits(draught_int, 8)

        binary += AISBinaryEncoder._encode_string(
            voyage.destination, 20
    ...
    def encode_type_18(vessel: VesselState) -> Tuple[str, Dict[str, Any]]:
        ...
        binary += AISBinaryEncoder._encode_bits(nav.raim, 1)  # RAIM
        # Radio status
        binary += AISBinaryEncoder._encode_bits(nav.radio_status, 20)
    ...
    @staticmethod
    def encode_type_21(
            aid_nav: AidToNavigationData) -> Tuple[str, Dict[str, Any]]:
        ...
        # Aid type
        binary += AISBinaryEncoder._encode_bits(aid_nav.aid_type, 5)
        # Name (120 bits)
        binary += AISBinaryEncoder._encode_string(aid_nav.name, 20)
        binary += AISBinaryEncoder._encode_bits(
            aid_nav.position_accuracy, 1
        )  # Position accuracy
        binary += AISBinaryEncoder._encode_longitude(
            aid_nav.position.longitude
        )  # Longitude
        binary += AISBinaryEncoder._encode_latitude(
            aid_nav.position.latitude
        )  # Latitude
        ...
        # EPFD type
        binary += AISBinaryEncoder._encode_bits(aid_nav.epfd_type.value, 4)
        # Time stamp
        binary += AISBinaryEncoder._encode_bits(aid_nav.timestamp, 6)
        # Off position
        binary += AISBinaryEncoder._encode_bits(aid_nav.off_position, 1)
        binary += AISBinaryEncoder._encode_bits(
            aid_nav.regional, 8
        )  # Regional reserved
        binary += AISBinaryEncoder._encode_bits(aid_nav.raim, 1)  # RAIM
        # Virtual aid
        binary += AISBinaryEncoder._encode_bits(aid_nav.virtual_aid, 1)
        # Assigned mode
        binary += AISBinaryEncoder._encode_bits(aid_nav.assigned, 1)
        binary += AISBinaryEncoder._encode_bits(0, 1)  # Spare
    ...
    @staticmethod
    def encode_type_24_part_a(
            vessel: VesselState) -> Tuple[str, Dict[str, Any]]:
        ...
        binary += AISBinaryEncoder._encode_bits(0, 2)  # Repeat indicator
        binary += AISBinaryEncoder._encode_bits(vessel.mmsi, 30)  # MMSI
        # Part number (0 = Part A)
        binary += AISBinaryEncoder._encode_bits(0, 2)
        binary += AISBinaryEncoder._encode_string(
            static.vessel_name, 20
        )  # Vessel name (120 bits)
    ...
    def encode_type_24_part_b(
        ...
        binary += AISBinaryEncoder._encode_bits(0, 2)  # Repeat indicator
        binary += AISBinaryEncoder._encode_bits(vessel.mmsi, 30)  # MMSI
        # Part number (1 = Part B)
        binary += AISBinaryEncoder._encode_bits(1, 2)
        # Ship type
        binary += AISBinaryEncoder._encode_bits(static.ship_type.value, 8)
        binary += AISBinaryEncoder._encode_string("", 3)  # Vendor ID (18 bits)
        # Unit model code (6 bits)
        binary += AISBinaryEncoder._encode_string("", 1)
