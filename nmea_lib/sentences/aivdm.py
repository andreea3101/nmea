...
class AIVDMSentence:
    ...
    def __init__(
        ...
        sequential_message_id: Optional[str] = None,
        channel: str = "A",
        payload: str = "",
        fill_bits: int = 0,
    ):
...
class AISMessageGenerator:
    ...
    def generate_type_1(
        self, vessel: VesselState, channel: str = "A"
    ...
    def generate_type_4(
        self, base_station: BaseStationData, channel: str = "A"
    ) -> Tuple[List[str], Dict[str, Any]]:
    ...
    def generate_type_5(
        ...
        sentences = AIVDMSentence.from_binary_message(
            binary_data, channel, seq_id)
        return [str(s) for s in sentences], input_data

    def generate_type_18(
        self, vessel: VesselState, channel: str = "B"
    ) -> Tuple[List[str], Dict[str, Any]]:
    ...
    def generate_type_21(
        self, aid_nav: AidToNavigationData, channel: str = "A"
    ) -> Tuple[List[str], Dict[str, Any]]:
    ...
    def generate_type_24(
        ...
        # Combine sentences
        all_sentences = [str(s) for s in sentences_a] + \
            [str(s) for s in sentences_b]

        # Combine input data
        combined_input = {"part_a": input_a, "part_b": input_b}

        return all_sentences, combined_input

    def generate_message(
        self, message_type: int, vessel_data: Any, channel: str = "A"
    ) -> Tuple[List[str], Dict[str, Any]]:
...
def validate_aivdm_sentence(sentence: str) -> bool:
    ...
    try:
        ...
        if not SentenceValidator.is_valid(
                sentence):  # Use SentenceValidator.is_valid
            return False

        # Parse sentence
        if not sentence.startswith(
            "!AIVDM,"
        ):  # This check is somewhat redundant due to SentenceValidator but good for clarity
            return False
