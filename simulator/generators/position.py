...
class PositionState:
    ...
    def __str__(self) -> str:
        return f"PositionState({
            self.position}, {
            self.speed.value:.1f}kts, {
            self.heading.value:.1f}°)"
...
class PositionGenerator:
    ...
    def __init__(
        ...
        # current_speed and current_heading are the speed/heading for the
        # *next* interval
        ...
        initial_state = PositionState(
            ...
            # For the initial state, these are the provided initial
            # speed/heading.
    ...
    def update_position(
        self, elapsed_seconds: float, timestamp: datetime
    ) -> PositionState:
        """
    ...
    def _apply_movement_variation(self) -> None:
        ...
        if self.course_variation > 0:
            ...
            new_heading = (self.current_heading.value +
                           course_change * 0.1) % 360
    ...
    def get_distance_traveled(self) -> float:
        ...
        # Needs at least two states (e.g., initial and one updated state) to
        # calculate distance.
        if len(self.position_history) < 2:
            return 0.0

        total_distance = 0.0
        # Iterate through segments: (pos_hist[0] to pos_hist[1]), (pos_hist[1]
        # to pos_hist[2]), ...
    ...
    def get_average_speed(self) -> float:
        ...
        # Total time from the timestamp of the initial state to the timestamp
        # of the last recorded state.
        total_time = (
        ...
        # This could happen if only one update occurred with elapsed_seconds =
        # 0.
        if total_time <= 0:
            ...
            # or based on the speed of the first state if only one state exists
            # (handled by len < 2).
            return 0.0

        distance_m = (
            self.get_distance_traveled()
        )  # This now uses the corrected distance logic
