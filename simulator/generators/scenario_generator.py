...
class CompleteScenarioGenerator:
    ...
    def _create_vessels(self):
        ...
        vessel_templates = [
            {
                "type": "container_ship",
                "class": VesselClass.CLASS_A,
                "ship_type": ShipType.CARGO_ALL_SHIPS,
            },
            {
                "type": "tanker",
                "class": VesselClass.CLASS_A,
                "ship_type": ShipType.TANKER_ALL_SHIPS,
            },
            {
                "type": "fishing_vessel",
                "class": VesselClass.CLASS_A,
            ...
            {
                "type": "pleasure_craft",
                "class": VesselClass.CLASS_B,
                "ship_type": ShipType.PLEASURE_CRAFT,
            },
            {
                "type": "cargo_ship",
                "class": VesselClass.CLASS_A,
        ...
        for i in range(self.config.vessel_count):
            ...
            vessel = create_vessel_state(
                ...
                # Ensure heading is int
                heading=int(random.uniform(0.0, 360.0)),
            ...
            if template["class"] == VesselClass.CLASS_A:
                vessel.voyage_data.destination = f"PORT_{
                    random.randint(
                        1, 10)}"
    ...
    def generate_scenario(self) -> Dict[str, str]:
        ...
        print(
            f"Generating scenario with {
                self.config.vessel_count} vessels..."
        )
        print(f"Duration: {self.config.duration_minutes} minutes")
        print(f"Output directory: {self.config.output_dir}")

        # Open output files
        nmea_file_path = Path(self.config.output_dir) / \
            self.config.nmea_filename
        human_readable_path = (
            Path(self.config.output_dir) / self.config.human_readable_filename
        )

        with open(nmea_file_path, "w") as nmea_file, open(
            human_readable_path, "w"
        ) as human_file:

            # Write headers
            human_file.write(
                "NMEA 0183 Scenario Generation - Human Readable Output\\n")
            human_file.write("=" * 80 + "\\n")
            human_file.write(
                f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\\n"
            )
            human_file.write(
                f"Scenario duration: {self.config.duration_minutes} minutes\\n"
            )
            ...
            while current_time < end_time:
                ...
                if (
                    self.config.include_gps
                    and (current_time - last_gps_time).total_seconds()
                    >= self.config.gps_interval_seconds
                ):

                    for vessel in self.vessels:
                        gps_sentences = self._generate_gps_sentences(
                            vessel, current_time
                        )
                        for sentence in gps_sentences:
                            nmea_file.write(sentence + "\\n")
                            self._add_reference_data(
                                sentence, "GPS", current_time, vessel.mmsi
                            )
                            self._write_human_readable(
                                human_file, sentence, "GPS", current_time, vessel)
                    ...
                    for vessel in self.vessels:
                        for msg_type, interval in self.config.ais_intervals.items():
                            ...
                            if key not in last_message_times:
                                ...
                                time_since_last = (
                                    current_time - last_message_times[key]
                                ).total_seconds()
                            ...
                            if should_send:
                                try:
                                    ...
                                    # as these types require BaseStationData
                                    # and AidToNavigationData respectively.
                                    if msg_type == 4 and isinstance(
                                        vessel, VesselState
                                    ):
                                        # print(f"Skipping AIS type 4 for Vessel {vessel.mmsi}, BaseStationData required.")
                                        continue
                                    if msg_type == 21 and isinstance(
                                        vessel, VesselState
                                    ):
                                    ...
                                    sentences, input_data = (
                                        self.ais_generator.generate_message(
                                            msg_type, vessel
                                        )
                                    )
                                    ...
                                    print(
                                        f"Error generating AIS type {msg_type} for vessel {
                                            vessel.mmsi}: {e}")

                # Progress indicator
                if step_count % 100 == 0:
                    progress = (
                        (current_time - self.config.start_time).total_seconds()
                        / (self.config.duration_minutes * 60)
                        * 100
                    )
                    print(
                        f"Progress: {progress:.1f}% - Generated {self.message_count} messages"
                    )

                # Advance time
                current_time += timedelta(
                    seconds=self.config.time_step_seconds)
        ...
        return {
            ...
            "reference_file": str(
                ...
                self.config.reference_filename),
    ...
    def _generate_gps_sentences(
        self, vessel: VesselState, current_time: datetime
    ) -> List[str]:
    ...
    def _add_reference_data(
        ...
        timestamp: datetime,
        vessel_mmsi: int,
        ais_msg_type: Optional[int] = None,
        input_data: Optional[Dict[str, Any]] = None,
    ):
    ...
    def _write_human_readable(
        ...
        if msg_type == "GPS":
            if sentence.startswith("$GPGGA"):
                file.write(
                    f"GPS Fix Data - Vessel {vessel.mmsi} ({vessel.static_data.vessel_name})\\n"
                )
                file.write(
                    f"  Position: {
                        vessel.navigation_data.position.latitude:.6f}, {
                        vessel.navigation_data.position.longitude:.6f}\\n"
                )
                file.write(f"  Sentence: {sentence}\\n")
            elif sentence.startswith("$GPRMC"):
                file.write(
                    f"GPS Recommended Minimum - Vessel {vessel.mmsi}\\n")
                file.write(
                    f"  Speed: {
                        vessel.navigation_data.sog:.1f} knots, Course: {
                        vessel.navigation_data.cog:.1f}°\\n"
                )
                file.write(f"  Sentence: {sentence}\\n")

        elif msg_type == "AIS":
            file.write(
                f"AIS Type {ais_msg_type} - Vessel {vessel.mmsi} ({vessel.static_data.vessel_name})\\n"
            )
            ...
            if ais_msg_type in [1, 2, 3]:
                ...
                file.write(
                    f"  Destination: {
                        vessel.voyage_data.destination}\\n"
                )
                ...
                file.write(
                    f"  Speed: {
                        vessel.navigation_data.sog:.1f} knots\\n"
                )
    ...
    def _save_reference_data(self):
        ...
        reference_path = Path(self.config.output_dir) / \
            self.config.reference_filename
        ...
        data = {
            ...
            "statistics": {
                ...
                "gps_messages": len(
                    [r for r in self.reference_data if r.message_type == "GPS"]
                ),
                "ais_messages": len(
                    [r for r in self.reference_data if r.message_type == "AIS"]
                ),
                "message_types": {},
            },
        }
...
def generate_complete_scenario(
    config: Optional[ScenarioGenerationConfig] = None,
) -> Dict[str, str]:
