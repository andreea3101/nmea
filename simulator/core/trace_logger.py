...
class AISTraceLogger:
    ...
    def log_vessel_update(self, vessel_mmsi: int,
    ...
    def log_scheduling_event(
        self, vessel_mmsi: int, message_type: int, event_data: Dict[str, Any]
    ):
    ...
    def log_binary_encoding(
        ...
        entry = TraceEntry(
            ...
            data={
                ...
                "binary_data": (
                    binary_data[:100] + "..." if len(binary_data) > 100 else binary_data
                ),
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        if stats["start_time"]:
            ...
            stats["entries_per_second"] = stats["entries_logged"] / \
                max(1, runtime)
...
class TraceAnalyzer:
    ...
    def get_error_summary(self) -> Dict[str, Any]:
        ...
        error_entries = [
            entry for entry in self.entries if entry.event_type == "error"]

        error_summary = {
            "total_errors": len(error_entries),
            "vessels_with_errors": len(
                set(entry.vessel_mmsi for entry in error_entries)
            ),
            "error_types": {},
            "recent_errors": [],
        }
    ...
    def export_vessel_data(self, vessel_mmsi: int, output_file: str):
        ...
        for entry in self.get_vessel_messages(vessel_mmsi):
            ...
            if entry.event_type == "vessel_updated" and entry.data:
                position_data = {"timestamp": entry.timestamp, **entry.data}
...
def create_trace_logger(
    log_file: str = None, enable_console: bool = False
) -> AISTraceLogger:
...
def generate_trace_report(trace_file: str, output_file: str):
    ...
    report = {
        "summary": {
            ...
            "time_range": {
                "start": (
                    min(entry.timestamp for entry in analyzer.entries)
                    if analyzer.entries
                    else None
                ),
                "end": (
                    max(entry.timestamp for entry in analyzer.entries)
                    if analyzer.entries
                    else None
                ),
            },
