...
class TCPOutput(OutputHandler):
    ...
    def start(self) -> None:
        ...
        try:
            ...
            self.client_manager_thread = threading.Thread(
                target=self._client_manager_loop, daemon=True
            )
            self.client_manager_thread.start()

            print(
                f"TCP server started on {
                    self.config.host}:{
                    self.config.port}"
            )
    ...
    def __str__(self) -> str:
        ...
        return f"TCPOutput({status}, {
            self.config.host}:{
            self.config.port}, {client_count} clients, {
