...
class SerialOutputConfig:
    ...
    max_reconnect_attempts: int = (
        5  # Maximum number of reconnect attempts (-1 for infinite)
    )
    ...
    def __post_init__(self):
        ...
        # This allows configuration from YAML/JSON using strings like
        # "EIGHTBITS", "PARITY_ODD", etc.
        if isinstance(self.bytesize, str):
            self.bytesize = getattr(
                serial, self.bytesize.upper(), serial.EIGHTBITS)
        if isinstance(self.parity, str):
            self.parity = getattr(
                serial, f"PARITY_{self.parity.upper()}", serial.PARITY_NONE
            )
        if isinstance(self.stopbits, str):
            self.stopbits = getattr(
                serial,
                f"STOPBITS_{self.stopbits.upper().replace('.', '_')}",
                serial.STOPBITS_ONE,
            )
        ...
        if not isinstance(self.line_ending, str):
            raise ValueError(
                "Line ending must be a string (e.g., '\\r\\n', '\\n').")
        try:
...
class SerialOutput(OutputHandler):
    ...
    def start(self) -> None:
        ...
        print(
            f"Attempting to start serial output on {
                self.config.port} at {
        ...
        try:
            ...
            # raise RuntimeError(f"Failed to start serial output: {e}") # Or
    ...
    def _connect(self) -> None:
        ...
        try:
            self.serial_port = serial.Serial(
                **self.config.get_serial_options())
    ...
    def stop(self) -> None:
        ...
        if self._reconnect_thread and self._reconnect_thread.is_alive():
            self._reconnect_thread.join(
                timeout=self.config.reconnect_delay + 1)
        ...
        with self._lock:
            ...
            try:
                ...
                self.last_sentence_time = (
                    time.time()
                )  # Record time of last successful send
                ...
                print(
                    f"Unexpected error sending data on {
                        self.config.port}: {e}"
                )
                self._handle_send_error()
                return False
    ...
    def _handle_send_error(self):
        ...
        self.is_running = (
            False  # Mark as not running to stop further sends until reconnected
        )
        if self.serial_port:
            try:
                self.serial_port.close()
    ...
    def _reconnect_loop(self) -> None:
        ...
        while not self._stop_event.is_set() and (
            ...
            if (
                self.is_running
            ):  # Should not happen if called from error, but good check
                break

            attempts += 1
            print(
                f"Reconnection attempt {attempts}/{
                    max_attempts if max_attempts != -1 else 'infinite'} for {
                    self.config.port}..."
            )
            ...
            try:
                ...
                print(
                    f"Unexpected error during reconnect attempt {attempts}: {e}")
                self._stop_event.wait(self.config.reconnect_delay)

        if not self.is_running and not self._stop_event.is_set():
            print(
                f"Failed to reconnect to {
                    self.config.port} after {attempts} attempts. Stopping reconnection attempts.")
            ...
            status.update(
                {
                    ...
                    "reconnecting": (
                        self._reconnect_thread.is_alive()
    ...
    def __str__(self) -> str:
        ...
        return f"SerialOutput({state}, {details}, {
            self.sentences_sent} sentences)"
