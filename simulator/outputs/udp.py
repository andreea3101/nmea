...
class UDPOutput(OutputHandler):
    ...
    def start(self) -> None:
        ...
        try:
            ...
            if self.config.multicast_group:
                print(
                    f"UDP multicast output started to {
                        self.config.multicast_group}:{
                        self.config.port}"
                )
            else:
                print(
                    f"UDP broadcast output started to {
                        self.config.host}:{
                        self.config.port}"
                )
        ...
        status.update(
            {
                ...
                "multicast_ttl": (
                    self.config.multicast_ttl if self.config.multicast_group else None
                ),
            }
        )
    ...
    def __str__(self) -> str:
        ...
        return f"UDPOutput({status}, {mode}, {targets} targets, {
