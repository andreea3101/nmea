...
class FileOutput(OutputHandler):
    ...
    def start(self) -> None:
        ...
        try:
            ...
            self.file_handle = open(
                self.config.file_path, mode, encoding="utf-8")
            ...
            if not self.config.append_mode or self.bytes_written == 0:
                header = f"# NMEA Simulation started at {
                    self.file_start_time.isoformat()}\n"
    ...
    def stop(self) -> None:
        ...
        try:
            if self.file_handle:
                ...
                footer = f"# NMEA Simulation ended at {
    ...
    def _check_rotation(self) -> None:
        ...
        if (
            self.config.rotation_size_mb
            and self.bytes_written > self.config.rotation_size_mb * 1024 * 1024
        ):
            needs_rotation = True

        # Check time-based rotation
        if (
            self.config.rotation_time_hours
            and (datetime.now() - self.file_start_time).total_seconds()
    ...
    def _rotate_file(self) -> None:
        ...
        try:
            ...
            # Open new file
            self.file_handle = open(
                self.config.file_path, "w", encoding="utf-8")
            self.bytes_written = 0
            self.file_start_time = datetime.now()

            # Write header
            header = f"# NMEA Simulation continued at {
                self.file_start_time.isoformat()}\n"
            self.file_handle.write(header)
            self.bytes_written += len(header.encode("utf-8"))
            ...
            try:
                self.file_handle = open(
                    self.config.file_path, "a", encoding="utf-8")
            except Exception:
                self.is_running = False
        ...
        try:
            ...
            for old_file in rotated_files[self.config.max_files:]:
                try:
                    ...
                    print(
                        f"Warning: Could not delete old file {old_file}: {e}")
