...
class MultiVesselConfigManager:
    ...
    def add_vessel_to_scenario(
            self, scenario_name: str, vessel_config: Dict[str, Any]):
    ...
    def add_base_station_to_scenario(
        self, scenario_name: str, base_station_config: Dict[str, Any]
    ):
    ...
    def add_aid_to_navigation_to_scenario(
        self, scenario_name: str, aid_nav_config: Dict[str, Any]
    ):
    ...
    def create_vessel_from_template(self,
                                    template_name: str,
                                    mmsi: int,
                                    name: str,
                                    position: Position,
    ...
    def create_fleet_scenario(
        self, name: str, fleet_config: Dict[str, Any]
    ) -> ScenarioConfig:
        """Create a scenario with multiple vessels (fleet)."""
        scenario = self.create_scenario(
            name, fleet_config.get("description", f"Fleet scenario: {name}")
        )
    ...
    def create_random_fleet(
        ...
        scenario = self.create_scenario(
            name, f"Random fleet with {vessel_count} vessels"
        )
        scenario.area = area
        ...
        for i in range(vessel_count):
            ...
            vessel_config = self.create_vessel_from_template(
                template, mmsi, f"RANDOM_{i:03d}", position, initial_heading=heading
            )
    ...
    def validate_scenario(self, scenario_name: str) -> List[str]:
        ...
        for i, vessel in enumerate(scenario.vessels):
            vessel_issues = self._validate_vessel_config(
                vessel, f"vessel[{i}]")
            issues.extend(vessel_issues)

        return issues

    def _validate_vessel_config(
            self, config: Dict[str, Any], prefix: str) -> List[str]:
        ...
        if ship_type is not None and not (0 <= ship_type <= 99):
            issues.append(
                f"{prefix}: Invalid ship type {ship_type} (must be 0-99)")
...
def create_english_channel_scenario() -> Dict[str, Any]:
    ...
    return {
        ...
        "area": {"lat_min": 50.5, "lat_max": 51.0, "lon_min": 1.0, "lon_max": 2.0},
        "vessels": [
            {
                "template": "container_ship",
                "mmsi": 235001234,
